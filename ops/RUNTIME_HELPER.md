...
Tunel jest zdalnie zarządzany. Po lokalnym smoke dodać Published application:

```text
ogniskowy-grajek.klikfirma.pl -> http://localhost:8501
```

Zachować istniejące trasy `api.klikfirma.pl -> 8000`, `gra.klikfirma.pl -> 8080` i końcowy 404.
Nie restartować `cloudflared` i nie rotować connector tokenu w tym wdrożeniu. Po zmianie sprawdzić HTTPS,
WebSocket Streamlit oraz oba stare hosty.
...
- Publikacja: usunąć tylko nową Published application/DNS, potem zatrzymać nową usługę.
